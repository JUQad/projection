...
void loaddata(std::string filename, sparseData& data, bool shuffle)
{
    ...
    // second pass: load contents
    {
        ...
        for (std::size_t i=0; i<n; i++)
        {
            ...
            // extract label
            std::size_t start = 0;
            std::size_t end = std::min(s.find(' ', start), s.size());
            string y = s.substr(start, end - start);
            int i_dec = std::stoi (y);
            if (i_dec%2 == 0)
            ...
            while (start < s.size())
            {
                ...
                if (colon == std::string::npos) throw runtime_error(string("[loaddata] failed to load file ").append(filename));
                end = std::min(s.find(' ', colon), s.size());
...
tuple<vector<double>, vector<double>, vector<double> > load_lookup (std::string filename)
{
    size_t size         = 400;
    size_t n            = 0;
    
    vector<double> wdValue;
    std::vector<double> mValue(size+1);
    std::vector<double> kernelValue(size+1);
