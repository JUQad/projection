...
class SVM {
public:
	SVM(std::vector<double>& dual_variables, sparseData& data, Kernel& kernel)
	: m_kernel(&kernel)
	{
	...
	double evaluateTestset(const sparseData& test_data) const {
        ...
        //double success_rate = ((double)number_of_errors/(double)test_data.labels.size());
		return success_rate;
	}
...
//double compareSVMs(SVM_newDS& svm1, SVM_newDS& svm2, sparseData& test_data);
