...
// --gamma
    ...
    //mergeHeuristicReprocessLASVM
    else if (heuristic_number == 7) {
        cout << "heuristic = 7, Budget maintanenace: mergeHeuristicReprocessLASVM" << endl;
	...
	size_t max_repeat = 1;
		if (test_path != "NOPATH")
        {
			...
			for (INDEX rep_i = 0; rep_i < max_repeat; rep_i++)
            {
                ...
                //SVM svm = solver(train_data, test_data, C, kernel, wd_parameters, accuracy, budget, epochs, HeuristicWithmoreVectors);
